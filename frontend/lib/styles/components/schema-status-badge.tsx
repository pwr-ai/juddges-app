/**
 * Schema Status Badge Component
 * Displays schema lifecycle status with semantic status tokens
 */

"use client";

import React from 'react';
import { cn } from '@/lib/utils';
import { SchemaStatus } from '@/types/extraction_schemas';
import { getStatusTone, STATUS_TONE_STYLES, StatusTone } from '@/lib/styles/status-tone';
import { FileText, CheckCircle2, Eye, Archive, Globe } from 'lucide-react';
import { Tooltip, TooltipContent, TooltipTrigger } from './tooltip';

/**
 * Props for SchemaStatusBadge component
 */
export interface SchemaStatusBadgeProps {
  /** Schema status value */
  status: SchemaStatus;
  /** Optional className for additional styling */
  className?: string;
  /** Size variant */
  size?: 'sm' | 'md';
}

/**
 * Schema Status Badge Component
 *
 * Displays schema lifecycle status with semantic status tokens:
 * - draft: gold
 * - published: ink
 * - review: gold
 * - archived: oxblood
 */
export function SchemaStatusBadge({
  status,
  className,
  size = 'sm',
}: SchemaStatusBadgeProps): React.JSX.Element {
  const statusConfig: Record<SchemaStatus, { label: string; icon: React.ComponentType<{ className?: string }>; tone: StatusTone }> = {
    draft: {
      label: 'Draft',
      icon: FileText,
      tone: 'gold',
    },
    published: {
      label: 'Published',
      icon: Globe,
      tone: 'ink',
    },
    review: {
      label: 'Review',
      icon: Eye,
      tone: 'gold',
    },
    archived: {
      label: 'Archived',
      icon: Archive,
      tone: 'oxblood',
    },
  };

  const config = statusConfig[status] ?? {
    label: status,
    icon: FileText,
    tone: getStatusTone(status),
  };
  const Icon = config.icon;
  const toneStyles = STATUS_TONE_STYLES[config.tone];

  return (
    <span
      className={cn(
        'inline-flex items-center gap-1.5 font-mono border border-rule rounded-full bg-transparent',
        toneStyles.text,
        size === 'sm' && 'text-xs px-2 py-0.5',
        size === 'md' && 'text-sm px-2.5 py-1',
        className
      )}
    >
      <Icon className={cn(
        size === 'sm' && 'h-3 w-3',
        size === 'md' && 'h-3.5 w-3.5'
      )} />
      <span>{config.label}</span>
    </span>
  );
}

/**
 * Verified Badge Component
 * Displays a checkmark badge indicating field-expert approval
 */
export interface VerifiedBadgeProps {
  /** Optional className for additional styling */
  className?: string;
  /** Size variant */
  size?: 'sm' | 'md' | 'lg';
}

/**
 * Verified Badge Component
 *
 * Displays a checkmark icon to indicate that a schema has been
 * approved by field experts. Shows "Expert verified" tooltip on hover.
 */
export function VerifiedBadge({
  className,
  size = 'lg',
}: VerifiedBadgeProps): React.JSX.Element {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          className={cn(
            'inline-flex items-center justify-center',
            'rounded-full',
            'border border-rule bg-transparent',
            'text-ink',
            'transition-colors hover:border-ink',
            size === 'sm' && 'h-5 w-5',
            size === 'md' && 'h-6 w-6',
            size === 'lg' && 'h-7 w-7',
            className
          )}
        >
          <CheckCircle2 className={cn(
            'text-ink',
            size === 'sm' && 'h-3 w-3',
            size === 'md' && 'h-4 w-4',
            size === 'lg' && 'h-5 w-5'
          )} />
        </div>
      </TooltipTrigger>
      <TooltipContent side="right">
        Expert verified
      </TooltipContent>
    </Tooltip>
  );
}
