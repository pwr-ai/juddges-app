...
@pytest.mark.search
@pytest.mark.integration
async def test_get_similar_documents_response_structure(authenticated_client: AsyncClient):
...
async def test_get_similar_documents_no_duplicates(authenticated_client: AsyncClient):
    ...
    if list_response.status_code == 200:
        ...
        if docs:
            ...
            assert len(doc_ids) == len(set(doc_ids)), "Similar documents contain duplicates"
...
@pytest.mark.search
@pytest.mark.integration
async def test_batch_similar_documents_nonexistent_ids(authenticated_client: AsyncClient):
...
@pytest.mark.search
@pytest.mark.integration
async def test_batch_similar_documents_mixed_valid_invalid(authenticated_client: AsyncClient):
...
@pytest.mark.api
@pytest.mark.search
async def test_batch_similar_documents_missing_fields(authenticated_client: AsyncClient):
...
@pytest.mark.search
@pytest.mark.integration
async def test_get_similar_documents_concurrent_requests(authenticated_client: AsyncClient):
...
async def test_similar_documents_consistency(authenticated_client: AsyncClient):
    ...
    if list_response.status_code == 200:
        ...
        if docs:
            ...
            similar1 = data1.get("documents", data1) if isinstance(data1, dict) else data1
            similar2 = data2.get("documents", data2) if isinstance(data2, dict) else data2
