...
class CourtHearingData(BaseModel):
    ...
    case_number_hearing: Optional[str] = Field(
        None,
        description="Case number at hearing",
        alias="caseNumber"
    )
    appeal_date: Optional[date] = Field(
        None,
        description="Date of appeal court judgment"
    )
    ...
    offender_representative: Optional[str] = Field(
        None,
        description="Name of offender's legal representative"
    )
    crown_representative: Optional[str] = Field(
        None,
        description="Name of Crown/Attorney General representative"
    )
...
class PleaInformation(BaseModel):
    ...
    confessed: Optional[bool] = Field(None, description="Did offender confess/plead guilty?")
    plea_point: Optional[Literal[
        "police_presence",
        ...
        "dont_know"
    ]] = Field(None, description="At what point during proceedings did offender plead guilty?")


class OffenderInformation(BaseModel):
    """Offender demographic and status information."""

    gender: Optional[Literal["male", "female", "all_male", "all_female", "male_and_female"]] = Field(
        None,
        description="Offender(s) gender"
    )
    age_at_offence: Optional[int] = Field(
        None,
        description="Offender(s) age at time of offence",
        ge=0,
        le=150
    )
    ...
    accommodation_status: Optional[Literal[
        "fixed_address",
        "homeless",
    ...
    victim_relationship: Optional[Literal[
        "stranger",
        "relative",
        "acquaintance",
        "dont_know"
...
class VictimInformation(BaseModel):
    ...
    age_at_offence: Optional[int] = Field(
        None,
        description="Victim(s) age at time of offence",
        ge=0,
        le=150
    )
    employment_status: Optional[Literal[
...
class OffenceTrialData(BaseModel):
    ...
    conviction_courts: Optional[List[str]] = Field(
        None,
        description="Court name(s) where offender convicted/pled guilty"
    )
    conviction_dates: Optional[List[str]] = Field(
        None,
        description="Conviction/guilty plea date(s) in ISO format"
    )
    convicted_offences: Optional[List[str]] = Field(
        None,
        description="Offence(s) offender was convicted of"
    )
    acquitted_offences: Optional[List[str]] = Field(
        None,
        description="Offence(s) offender was acquitted of"
    )
    ...
    # Remand information
    remand_decision: Optional[Literal[
        "unconditional_bail",
        "conditional_bail",
        "remanded_custody",
        "dont_know"
    ...
    remand_custody_duration: Optional[str] = Field(
        None,
        description="Duration of custody remand"
    )

    # Sentencing details
    sentence_court: Optional[str] = Field(None, description="Court where offender sentenced")
    sentences: Optional[List[str]] = Field(None, description="Sentence(s) received")
    sentence_serve_type: Optional[Literal[
        "all_concurrent",
        "all_consecutive",
        "combination",
        "dont_know"
    ...
    defence_evidence: Optional[List[str]] = Field(
        None,
        description="Types of evidence defence presented at trial"
    )

    # Sentencing factors
    pre_sentence_report: Optional[Literal["low", "medium", "high", "dont_know"]] = Field(
        None,
        description="Pre-sentence report risk assessment"
    )
    aggravating_factors: Optional[List[str]] = Field(
        None,
        description="Aggravating factors mentioned by court"
    )
    mitigating_factors: Optional[List[str]] = Field(
        None,
        description="Mitigating factors mentioned by court"
    )
    victim_impact_statement: Optional[bool] = Field(
        None,
        description="Was victim impact statement given at sentencing?"
    )
...
class AppealReasons(BaseModel):
    ...
    quash_conviction: Optional[List[str]] = Field(
        None,
        description="Reasons why conviction is unsafe/quashed"
    )
    sentence_excessive: Optional[List[str]] = Field(
        None,
        description="Reasons why sentence is unduly excessive"
    )
    sentence_lenient: Optional[List[str]] = Field(
        None,
        description="Reasons why sentence is unduly lenient"
    )
    dismissed: Optional[List[str]] = Field(
        None,
        description="Reasons why appeal was dismissed/failed"
    )
...
class AppealData(BaseModel):
    ...
    appellant: Optional[Literal["offender", "attorney_general", "other"]] = Field(
        None,
        description="Who is the appellant?"
    )
    co_defendants: Optional[CoDefendantInfo] = None

    appeal_against: Optional[Literal[
        "conviction_unsafe",
        "sentence_excessive",
        "sentence_lenient",
    ...
    sentencing_guidelines: Optional[List[str]] = Field(
        None,
        description="Sentencing guidelines/laws/acts mentioned by appeal court"
    )

    appeal_outcome: Optional[Literal[
        "dismissed",
    ...
    reasons: Optional[AppealReasons] = Field(
        None,
        description="Appeal court's reasoning for the decision"
    )
...
class Judgment(BaseModel):
    ...
    language: Optional[str] = Field(None, description="Language code (ISO 639-1): pl, en, uk")
    ...
    judges: Optional[Dict[str, Any]] = Field(
        None,
        description="Judge information (names, roles)"
    )
    ...
    decision_type: Optional[str] = Field(
        ...
        examples=["Judgment", "Order", "Ruling"]
    )
    outcome: Optional[str] = Field(
        None,
        description="Case outcome",
        examples=["Granted", "Dismissed", "Remanded"]
    )

    # Classification
    keywords: Optional[List[str]] = Field(default_factory=list, description="Keywords")
    legal_topics: Optional[List[str]] = Field(default_factory=list, description="Legal topics")
    cited_legislation: Optional[List[str]] = Field(
        default_factory=list,
        description="Cited legislation/acts"
    )

    # Vector embedding
    embedding: Optional[List[float]] = Field(
        None,
        description="1536-dimensional embedding vector for semantic search"
    )

    # Coding scheme data (flexible JSONB fields)
    court_hearing_data: Optional[CourtHearingData] = Field(
        None,
        description="Court hearing details (Section 2)"
    )
    offence_trial_data: Optional[OffenceTrialData] = Field(
        None,
        description="Offence/trial/sentence details (Section 3)"
    )
    appeal_data: Optional[AppealData] = Field(
        None,
        description="Appeal information (Section 4)"
    )
...
class HybridSearchResult(Judgment):
    ...
    vector_score: Optional[float] = Field(None, description="Vector similarity score (0-1)")
