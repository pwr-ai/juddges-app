...
async def test_create_schema(
    ...
    sample_schema_data: Dict[str, Any]
):
...
async def test_create_schema_minimal(authenticated_client: AsyncClient):
    ...
    minimal_schema = {
        "name": f"MinimalSchema_{uuid.uuid4().hex[:8]}",
        "fields": []
    }
...
async def test_delete_schema(authenticated_client: AsyncClient):
    ...
    # Create a schema
    schema_data = {
        "name": f"DeleteTest_{uuid.uuid4().hex[:8]}",
        "fields": []
    }
    ...
    if gen_response.status_code in [200, 201]:
        ...
        if session_id:
            ...
            refinement_request = {
                ...
                "modifications": {
                    "add_field": {"name": "new_field", "type": "string"}
                }
            }
            
            response = await authenticated_client.post(
                f"/schemas/generate/{session_id}/refine",
...
async def test_schema_versioning(authenticated_client: AsyncClient):
    ...
    if create_response.status_code in [200, 201]:
        ...
        # Get versions
        response = await authenticated_client.get(
            f"/schemas/db/{schema_id}/versions"
        )
