...
class JudgmentIngestionPipeline:
    ...
    def __init__(
        ...
        self.transformers_url = transformers_url or os.getenv(
    ...
    def generate_embedding(self, text: str) -> Optional[List[float]]:
        ...
        Returns:
            List of 1024 floats (BAAI/bge-m3 embedding), or None if service unavailable
        """
        try:
    ...
    def ingest_polish_judgments(self, sample_size: int = 100) -> int:
        ...
        print(
            f"   Target: {sample_size} (filtering for Sąd Apelacyjny + Wydział Karny)"
        )
        ...
        try:
            ...
            print(
                f"✅ Successfully ingested {ingested} Polish criminal appellate judgments"
            )
            return ingested
        ...
        try:
            ...
            for case in tqdm(sample_data, desc="Ingesting en-court-raw judgments"):
                judgment_data = self._transform_uk_judgment(
                    case, source="JuDDGES/en-court-raw"
                )
                if judgment_data:
                    self._insert_judgment(judgment_data)
                    total_ingested += 1
        ...
        if remaining > 0:
            ...
            try:
                ...
                for case in tqdm(sample_data2, desc="Ingesting Appeal Court judgments"):
                    judgment_data = self._transform_uk_judgment(
                        case, source="JuDDGES/en-appealcourt"
                    )
                    if judgment_data:
                        self._insert_judgment(judgment_data)
                        total_ingested += 1
    ...
    def _transform_uk_judgment(
        self, raw_data: Dict, source: str = "JuDDGES/en-appealcourt"
    ) -> Optional[Dict]:
        """
    ...
    def _insert_judgment(self, judgment_data: Dict) -> bool:
        ...
        try:
            ...
            print(
                f"Warning: Failed to insert judgment {judgment_data.get('case_number')}: {e}"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ingest judgments from HuggingFace into Supabase"
    )
    parser.add_argument(
        "--polish", type=int, default=0, help="Number of Polish judgments to ingest"
    )
    parser.add_argument(
        "--uk", type=int, default=0, help="Number of UK judgments to ingest"
    )
    parser.add_argument(
        "--skip-polish", action="store_true", help="Skip Polish judgments"
    )
    parser.add_argument("--skip-uk", action="store_true", help="Skip UK judgments")
    parser.add_argument(
        "--no-embeddings", action="store_true", help="Skip generating embeddings"
    )
    ...
    )  # Need service role for writes
    transformers_url = (
        os.getenv("TRANSFORMERS_INFERENCE_URL", "http://localhost:8080")
    ...
    if not supabase_url or not supabase_key:
        ...
        print(
            "Optional: TRANSFORMERS_INFERENCE_URL (for embeddings, default: http://localhost:8080)"
        )
        sys.exit(1)
