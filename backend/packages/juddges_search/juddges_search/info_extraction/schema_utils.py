...
def parse_field_without_nested_fields(values: dict[str, Any], language: str) -> dict[str, Any]:
    ...
    if "type" not in values:
        logger.error(
            f"Field is missing required 'type' field. Cannot parse field correctly without type. Field values: {values}"
        )
        raise SchemaProcessingError(
            f"Field is missing required 'type' field. Cannot parse field correctly without type. Field values: {values}"
        )
...
def parse_field_with_nested_fields(values: dict[str, Any], language: str) -> dict[str, Any]:
    ...
    if "type" not in values:
        logger.error(
            f"Field with nested properties is missing required 'type' field. Cannot parse field correctly without type. Field values: {values}"
        )
        raise SchemaProcessingError(
    ...
    if not isinstance(nested_properties, dict):
        ...
        raise ValueError(
            f"Field with nested properties must have 'properties' as a dict, got {type(nested_properties)}"
        )
    ...
    logger.debug(
        f"parse_field_with_nested_fields returning properties with {len(properties.get('properties', {}))} nested fields, {len(required_field_names)} required"
    )
    return properties


def convert_schema_to_oai_structured_output(
    schema: dict[str, Any], language: str, strict: bool = True
) -> dict[str, Any]:
    """
    ...
    for field, field_values in text_schema.items():
        ...
        if "properties" not in field_values:
            ...
            field_type = field_values.get("type") if isinstance(field_values, dict) else type(field_values).__name__
            logger.error(f"Failed to parse field '{field}' with type '{field_type}'. Field values: {field_values}")
            raise SchemaProcessingError(
