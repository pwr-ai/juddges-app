...
test.describe('precedents cohort contract', () => {
  ...
  test('renders the grouped block and filters the ranked list on a bar click', async ({
    ...
    // COHORT_GROUP_FIELDS). The stub's `dismissed` isn't a real
    // appeal_outcome enum token, so it only round-trips through
    // formatEnumOptionLabel's capitalization fallback, not prefix-stripping.
    ...
    //
    // This locator reaches into Plotly's internal DOM (`g.point`,
    // `data-unformatted`) and clicks with `force: true`, skipping the
    // actionability check — the only spec in this gate that does either.
    // `frontend/package.json` pins `"plotly.js-dist": "^4.1.0"`, a caret
    // range, so a lockfile refresh can move Plotly's internals without any
    // app-code change. If this spec starts failing after a dependency bump
    // with no app change, check here first.
    const dismissedBar = block
      .locator('g.point', { has: page.locator('text[data-unformatted="3"]') })
      .locator('path');
    await expect(dismissedBar).toHaveCount(1);
    await dismissedBar.click({ force: true });

    // The filter chip names the active value and how many of the 2 ranked
    // results survive it — only `p-dismissed` carries `dismissed`.
    await expect(block).toContainText('Filtered to Dismissed');
    await expect(block).toContainText('1 of 2 ranked results');
