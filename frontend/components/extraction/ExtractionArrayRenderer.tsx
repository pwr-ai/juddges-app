  ...
  if (isArrayOfObjects(value)) {
    ...
    return (
      <div className={className}>
        ...
        <div className="rounded-lg border border-slate-200/60 dark:border-slate-700/60 bg-slate-50/50 dark:bg-slate-900/80 shadow-sm overflow-hidden">
  ...
  const listContent = (
    <ul className="space-y-2">
      {value.map((item, index) => {
        ...
        return (
          <li key={index} className="text-sm text-slate-900 dark:text-slate-100 leading-relaxed">
            • {displayValue}
          </li>
        );
      })}
