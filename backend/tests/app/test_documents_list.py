...
@pytest.mark.api
@pytest.mark.integration
async def test_list_documents_with_coordinates_filter(authenticated_client: AsyncClient):
...
@pytest.mark.api
@pytest.mark.integration
async def test_list_documents_without_coordinates_filter(authenticated_client: AsyncClient):
    """Test document listing without coordinates filter."""
    response = await authenticated_client.get(
        "/documents",
        params={
            "limit": 10,
            "only_with_coordinates": False
        }
    )
...
@pytest.mark.anyio
@pytest.mark.api
async def test_get_documents_sample_invalid_size_too_low(authenticated_client: AsyncClient):
...
@pytest.mark.anyio
@pytest.mark.api
async def test_get_documents_sample_invalid_size_negative(authenticated_client: AsyncClient):
...
async def test_list_documents_empty_database(authenticated_client: AsyncClient):
    ...
    response = await authenticated_client.get(
        "/documents",
        params={"limit": 10, "only_with_coordinates": False}
    )
...
@pytest.mark.anyio
@pytest.mark.api
async def test_list_documents_multiple_invalid_params(authenticated_client: AsyncClient):
    """Test listing documents with multiple invalid parameters."""
    response = await authenticated_client.get(
        "/documents",
        params={
            "limit": -5,
            "return_vectors": "not_a_boolean"
        }
    )
