"""Batch deep analysis for judgments using Anthropic models.

Performs deep analytical assessment: complexity scoring, reasoning quality,
legal doctrine classification, outcome determinative factors, and more.
...
Usage:
    ...
    # Use Opus for higher quality (recommended for deep analysis)
    python scripts/run_deep_analysis.py --model claude-opus-4-6

    # Adjust concurrency and batch size
    python scripts/run_deep_analysis.py --concurrency 10 --batch-size 100

    # Process only UK or PL
    python scripts/run_deep_analysis.py --jurisdiction UK

    # Retry failed only
    python scripts/run_deep_analysis.py --failed-only

    # Dry run (count without processing)
    python scripts/run_deep_analysis.py --dry-run

    # Limit total documents processed
    python scripts/run_deep_analysis.py --limit 100
"""
...
DEEP_SCHEMA_EN = SCHEMA_DIR / "deep_analysis_schema_en.json"
DEEP_SCHEMA_PL = SCHEMA_DIR / "deep_analysis_schema_pl.json"

STATUS_COLUMN = "deep_analysis_status"
RAW_COLUMN = "deep_analysis_raw"
MODEL_COLUMN = "deep_analysis_model"
ERROR_COLUMN = "deep_analysis_error"
TIMESTAMP_COLUMN = "deep_analysed_at"


def _default_checkpoint_path() -> Path:
    ts = datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
    return DATA_DIR / f"deep_analysis_checkpoint_{ts}.jsonl"


def _default_report_path() -> Path:
    ts = datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
    return DATA_DIR / f"deep_analysis_report_{ts}.json"
...
def load_instructions(language: str) -> str:
    ...
    instruction_file = PROMPTS_DIR / f"deep_analysis_instructions_{language}.yaml"
...
def build_extraction_prompt(full_text: str, language: str) -> str:
    ...
    context = (
        "Perform a deep analytical assessment of this court judgment using the provided schema. "
        "Analyse the complexity, reasoning quality, legal doctrine, and outcome-determinative factors. "
        "You are acting as a senior legal researcher."
    )

    labels = {
        "en": {"judgment": "Judgment text for deep analysis:"},
        "pl": {"judgment": "Tekst orzeczenia do pogłębionej analizy:"},
    }
...
def extract_queryable_fields(extracted_data: dict) -> dict:
    ...
    complexity = extracted_data.get("case_complexity", {})
    cross_ref = extracted_data.get("cross_reference_potential", {})
    doc_quality = extracted_data.get("document_quality", {})

    return {
        "deep_complexity_score": complexity.get("overall_score"),
        "deep_factual_complexity": complexity.get("factual_complexity"),
        "deep_legal_complexity": complexity.get("legal_complexity"),
        "deep_reasoning_quality_score": extracted_data.get("reasoning_quality_score"),
        "deep_legal_domains": extracted_data.get("legal_domains"),
        "deep_reasoning_patterns": extracted_data.get("reasoning_patterns"),
        "deep_judicial_tone": extracted_data.get("judicial_tone"),
        "deep_precedential_value": cross_ref.get("precedential_value"),
        "deep_research_value": cross_ref.get("research_value"),
        "deep_text_quality": doc_quality.get("text_quality"),
        "deep_analysis_confidence": extracted_data.get("analysis_confidence"),
    }
...
async def run_extraction(args):
    ...
    en_schema = clean_schema_for_anthropic(load_schema(DEEP_SCHEMA_EN))
    pl_schema = clean_schema_for_anthropic(load_schema(DEEP_SCHEMA_PL))
    ...
    logger.info("Fetching pending judgments for deep analysis...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run deep analysis on judgments"
    )
    parser.add_argument(
