...
class TestPydanticModels:
    ...
    def test_set_active_model_request(self):
        req = SetActiveModelRequest(model_id="tei/bge-m3")
        assert req.model_id == "tei/bge-m3"
    ...
    def test_set_active_model_response(self):
        resp = SetActiveModelResponse(
            ...
            dimensions=1024,
            message="Active embedding model set to tei/bge-m3",
        )
        assert resp.dimensions == 1024
        assert resp.provider == "tei"
    ...
    def test_test_embedding_request_with_model(self):
        req = TestEmbeddingRequest(text="hello", model_id="tei/bge-m3")
        assert req.model_id == "tei/bge-m3"
...
class TestListModelsEndpoint:
    ...
    async def test_list_models_success(
        ...
        assert data["active_model_id"] == "tei/bge-m3"
        assert len(data["models"]) == 1

    @patch("app.embeddings_api.get_default_model_id", return_value="tei/bge-m3")
...
class TestGetActiveModelEndpoint:
    ...
    async def test_get_active_model(
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["model_id"] == "tei/bge-m3"
        assert data["dimensions"] == 1024
        assert data["provider"] == "tei"
...
class TestSetActiveModelEndpoint:
    ...
    async def test_set_active_model_success(self, mock_set, client, valid_api_headers):
        ...
        mock_set.assert_called_once_with("tei/bge-m3")
...
class TestTestEmbeddingEndpoint:
    ...
    async def test_embed_with_explicit_model(
        ...
        resp = await client.post(
            "/embeddings/test",
            json={"text": "test", "model_id": "tei/bge-m3"},
            headers=valid_api_headers,
        )
        assert resp.status_code == 200
        mock_provider_fn.assert_called_once_with("tei/bge-m3")
