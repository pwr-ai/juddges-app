...
export interface BaseFiltersDrawerProps {
  ...
  defaultOpen?: boolean;
  /**
   * Hide the drawer's own "Filters" toggle and keep it permanently expanded.
   * For callers that already gate the drawer's mount behind their own
   * toggle, so there is only one expand/collapse control on screen.
   */
  hideToggle?: boolean;
}
...
export function BaseFiltersDrawer({
  ...
  const [open, setOpen] = React.useState(defaultOpen || hideToggle);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        {hideToggle ? (
          <span className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider text-[color:var(--ink-soft)]">
            {label}
          </span>
        ) : (
          <button
