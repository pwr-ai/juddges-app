...
describe('publications API contract', () => {
  ...
  const createRequest = {
    ...
    authors: backendPublication.authors,
    venue: backendPublication.venue,
    year: backendPublication.year,
  ...
  it('normalizes the FastAPI list response for frontend consumers', async () => {
    ...
    await expect(getPublications()).resolves.toEqual([expectedPublication]);
  });

  it.each([
    ['detail', () => getPublication('publication-1')],
    ['create', () => createPublication(createRequest)],
    [
      'update',
      () => updatePublication('publication-1', { title: 'Updated title' }),
    ],
  ])('normalizes the FastAPI %s response', async (_name, request) => {
    global.fetch = jest.fn(async () =>
      new Response(JSON.stringify(backendPublication), {
        status: 200,
        headers: { 'content-type': 'application/json' },
    ...
    await expect(request()).resolves.toEqual(expectedPublication);
  });
});
