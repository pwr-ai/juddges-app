    ...
    for row in ds:
        ...
        try:
            legal_doc = LegalDocument(
                ...
                judges=[str(j) for j in row.get("judges", []) if j]
                if isinstance(row.get("judges"), list)
                else [],
                legal_bases=[str(lb) for lb in row.get("legal_bases", []) if lb]
                if isinstance(row.get("legal_bases"), list)
                else [],
            ...
            records.append(
                (str(uuid.uuid4()), [0.0] * 1536, legal_doc.model_dump(mode="json"))
            )
    ...
    for row in ds:
        ...
        try:
            legal_doc = LegalDocument(
                ...
                full_text=text,
                case_type="criminal",
                metadata={"original_output": row.get("output")},
            )
