    ...
    >
      ...
      >
        ...
        {onCancel && (
          <button
            onClick={onCancel}
            className="absolute top-4 right-4 p-2 rounded-xl text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-white/30 dark:hover:bg-slate-800/50 backdrop-blur-sm transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-primary/50 z-20"
            type="button"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
        ...
        <div className="relative z-10">
              ...
              >
                {mode === "thinking" ? (
                  <span className="flex items-center justify-center gap-2">
                    Searching with <Sparkles className="h-5 w-5 text-blue-600 dark:text-blue-400" />
          ...
          {/* Progress Bar */}
          <div
            className="mt-6 h-1.5 bg-white/30 dark:bg-slate-800/40 rounded-full overflow-hidden backdrop-blur-sm border border-white/20 dark:border-slate-700/30"
            role="progressbar"
