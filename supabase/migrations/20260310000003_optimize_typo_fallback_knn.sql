-- =============================================================================
-- Migration: Optimize typo fallback with trigram KNN candidates
-- =============================================================================
-- Root causes observed after the previous optimization:
-- 1) misspelled queries still spent ~4-5s in fuzzy fallback
-- 2) similarity() over title/summary required broad scans on typo paths
--
-- This migration keeps the API contract unchanged while:
-- - adding GiST trigram indexes for KNN (<->) lookup
-- - narrowing fuzzy fallback to top candidates from title/summary indexes
-- - preserving vector search, FTS branches, and response schema
-- =============================================================================
...
COMMENT ON FUNCTION public.search_judgments_hybrid IS
    'Hybrid search optimized with index-friendly language branches, '
    'summary-weighted ranking, trigram KNN fuzzy fallback, and RRF fusion.';
