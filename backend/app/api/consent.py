...
from app.core.auth_jwt import AuthenticatedUser, get_admin_supabase_client, get_current_user
...
class ConsentStatusResponse(BaseModel):
    ...
    privacy_policy_accepted: bool = Field(description="Whether user accepted privacy policy")
    privacy_policy_accepted_date: Optional[str] = None
    privacy_policy_accepted_version: Optional[str] = None

    data_processing_consent: bool = Field(description="Whether user consented to data processing")
...
async def update_consent(
    ...
    try:
        ...
        consent_result = client.table("user_consent").select("*").eq(
            "user_id", user.id
        ).execute()
        ...
        if not consent_result.data:
            raise HTTPException(
                status_code=500,
                detail="Failed to retrieve updated consent status"
            )
        ...
        consent_status = ConsentStatusResponse(
            user_id=user.id,
            professional_acknowledgment_accepted=consent_data.get("professional_acknowledgment_accepted", False),
            professional_acknowledgment_date=consent_data.get("professional_acknowledgment_date"),
            professional_acknowledgment_version=consent_data.get("professional_acknowledgment_version"),
            ...
            privacy_policy_accepted_date=consent_data.get("privacy_policy_accepted_date"),
            privacy_policy_accepted_version=consent_data.get("privacy_policy_accepted_version"),
            data_processing_consent=consent_data.get("data_processing_consent", False),
            data_processing_consent_date=consent_data.get("data_processing_consent_date"),
...
async def get_consent_status(
    ...
    try:
        ...
        # Get consent status
        result = client.table("user_consent").select("*").eq("user_id", user.id).execute()

        if not result.data:
            # No consent record yet - return defaults
            logger.info(f"No consent record found for user {user.id}, returning defaults")
        ...
        logger.info(f"Retrieved consent status for user {user.id} (compliant: {is_compliant})")
...
async def get_consent_history(
    ...
    try:
        ...
        result = client.table("user_consent").select("consent_history").eq(
            "user_id", user.id
        ).execute()
        ...
        history_entries = [
            ConsentHistoryEntry(**entry) for entry in consent_history
        ]

        logger.info(f"Retrieved consent history for user {user.id}: {len(history_entries)} entries")

        return ConsentHistoryResponse(
            user_id=user.id,
            consent_history=history_entries
        )
