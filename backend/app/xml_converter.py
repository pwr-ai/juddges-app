...
def convert_xml_to_html(xml_content):
    ...
    try:
        ...
        def is_numbered_unit(element):
            ...
            if (element.tag == 'xUnit' and 
                element.get('xIsTitle', 'false').lower() == 'false' and
                element.get('xType') == 'none'):
                
                x_name = element.find('xName')
                x_text = element.find('xText')
                
                if x_name is not None and x_text is not None and x_name.text and x_text.text:
        ...
        def process_text_content(element, level=0):
            ...
            for child in element:
                if child.tag == 'xBx':
                    ...
                    # Anonymized content
                    child_content = process_text_content(child, level + 1)
                    content.append(f'<span class="anonymized">{"".join(child_content)}</span>')
                elif child.tag == 'xLexLink':
                    # Legal reference
                    child_content = process_text_content(child, level + 1)
                    title_attr = f' title="{child.get("xTitle")}"' if child.get('xTitle') else ''
                    content.append(f'<span class="legal-reference"{title_attr}>{"".join(child_content)}</span>')
                else:
        ...
        def process_element(element, level=0):
            ...
            if element.tag == 'xName':
                if element.text:
                    ...
                    result.append(f'<div class="element-xname">{"".join(content)}</div>')
                ...
                has_content = (element.text and element.text.strip()) or len(element) > 0
                ...
                content = process_text_content(element, level)
                title_attr = f' title="{element.get("xTitle")}"' if element.get('xTitle') else ''
                result.append(f'<span class="legal-reference"{title_attr}>{"".join(content)}</span>')
                ...
                class_attr = f' class="{" ".join(unit_classes)}"' if unit_classes else ''
            ...
            # Ignore xCOLGROUPx and xCOLx as they're handled in xRows
                ...
                if element.text and element.text.strip():
                    content = process_text_content(element, level)
                    result.append(f'<div class="element-{element.tag.lower()}">{"".join(content)}</div>')
                else:
