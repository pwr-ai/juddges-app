...
class TestExtractionCreation:
    ...
    async def test_create_extraction_job(
        ...
        client: AsyncClient,
        auth_headers: dict,
        sample_extraction_request: dict
    ):
...
class TestExtractionFromDB:
    ...
    @pytest.mark.anyio
    async def test_create_extraction_db(
        self,
        client: AsyncClient,
        auth_headers: dict
    ):
...
class TestExtractionRetrieval:
    ...
    async def test_get_extraction_job(
        ...
        job_id = create_response.json()["id"] if "id" in create_response.json() else create_response.json()["job_id"]
    ...
    async def test_get_nonexistent_extraction(
        ...
        response = await client.get(
            "/api/extractions/nonexistent-job-999",
            headers=auth_headers
        )
...
class TestExtractionListing:
    ...
    @pytest.mark.anyio
    async def test_list_extractions_without_auth(
        self,
        client: AsyncClient
    ):
...
class TestExtractionDeletion:
    ...
    @pytest.mark.anyio
    async def test_delete_extraction_job(
        self,
        client: AsyncClient,
        auth_headers: dict
    ):
