...
This module contains performance benchmarking tools for search query operations in the Juddges search system.

**Note**: This was originally designed for Weaviate in the AI-Tax platform. Juddges uses Supabase pgvector, so some functionality may be legacy.
...
python -m juddges_search.performance.cli --vector

# Run only hybrid search benchmarks  
python -m juddges_search.performance.cli --hybrid

# Run only BM25 search benchmarks
python -m juddges_search.performance.cli --bm25

# Limit to 5 queries per type
python -m juddges_search.performance.cli --queries 5

# Change max documents returned per query
python -m juddges_search.performance.cli --max-docs 20

# Specify custom output directory
python -m juddges_search.performance.cli --output-dir ./my_results
```

### Programmatic Usage

```python
from juddges_search.performance.weaviate_benchmark import WeaviateBenchmarkSuite
...
- Leverages existing search functions from `juddges_search.retrieval.weaviate_search`
