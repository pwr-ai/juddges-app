...
export function SchemaFieldsTable({ fields, onRowClick, onFieldUpdate, editable = false }: SchemaFieldsTableProps) {
        ...
        return (
          <div 
            className={cn(
              ...
              isNested && "border-l-2 border-l-primary/30 pl-3 ml-2 backdrop-blur-sm bg-primary/5 dark:bg-primary/10 rounded-r-md pr-2"
            )}
              ...
              <div className="flex flex-col gap-2 w-full min-w-[250px] max-w-[500px]" onClick={(e) => e.stopPropagation()}>
                <Input
                  ...
                  className={cn(
                    ...
                    "border-2 border-primary/50 dark:border-primary/50",
          ...
          <div className="py-1">
            <Badge
              ...
              className={cn(
                ...
                style.bg,
                style.border,
                "ring-slate-200/20 dark:ring-slate-700/30"
              )}
            >
              {style.icon}
              <span>{typeLabel}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 max-w-md relative">
          ...
          <Input
            ...
            className="pl-10 h-10 backdrop-blur-xl bg-white/60 dark:bg-slate-900/60 border-slate-200/50 dark:border-slate-700/50 shadow-lg shadow-slate-200/20 dark:shadow-slate-900/20 ring-1 ring-slate-200/20 dark:ring-slate-700/20 transition-all hover:bg-white/80 dark:hover:bg-slate-900/80 focus:ring-2 focus:ring-primary/20"
          />
        </div>
        {/* Stats - next to search input */}
        <div className="text-sm font-medium text-foreground/70 backdrop-blur-sm bg-white/40 dark:bg-slate-900/40 rounded-lg px-4 py-2 border border-slate-200/30 dark:border-slate-700/30 shadow-sm">
      ...
      {/* Table */}
      <div className="rounded-xl border border-slate-200/40 dark:border-slate-700/40 overflow-hidden backdrop-blur-2xl bg-gradient-to-br from-white/80 via-white/70 to-white/60 dark:from-slate-900/80 dark:via-slate-900/70 dark:to-slate-900/60 shadow-2xl shadow-slate-200/20 dark:shadow-slate-900/40 ring-1 ring-white/20 dark:ring-slate-700/30">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow 
                key={headerGroup.id}
                className="backdrop-blur-xl bg-gradient-to-r from-slate-50/90 via-slate-50/80 to-slate-50/90 dark:from-slate-800/90 dark:via-slate-800/80 dark:to-slate-800/90 border-b border-slate-200/40 dark:border-slate-700/40 shadow-sm"
              >
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row, index) => {
                ...
                <TableRow
                  ...
                  className={cn(
                    ...
                    "border-b border-slate-100/30 dark:border-slate-800/30",
                    isNested
                      ? "backdrop-blur-sm bg-primary/3 dark:bg-primary/5"
                      : index % 2 === 0 
                        ? "backdrop-blur-sm bg-white/50 dark:bg-slate-900/30" 
                        : "backdrop-blur-sm bg-slate-50/40 dark:bg-slate-800/20",
                    !editingCell && "hover:bg-gradient-to-r hover:from-primary/5 hover:via-primary/3 hover:to-transparent dark:hover:from-primary/10 dark:hover:via-primary/5 dark:hover:to-transparent hover:backdrop-blur-md hover:shadow-sm hover:border-primary/20 transition-all duration-200"
                  )}
                  onClick={() => {
                    if (!editingCell) {
              ...
              <TableRow>
                <TableCell
                  ...
                  className="h-24 text-center text-muted-foreground backdrop-blur-sm bg-white/30 dark:bg-slate-900/20"
                >
                  No fields found.
                </TableCell>
              </TableRow>
