...
async def test_get_document_metadata_structure(authenticated_client: AsyncClient):
    ...
    if list_response.status_code == 200:
        ...
        if docs:
            ...
            possible_fields = [
                "id", "title", "court_name", "case_number",
                "decision_date", "jurisdiction", "document_type",
                "judges", "keywords", "legal_bases"
            ]
...
@pytest.mark.api
@pytest.mark.integration
async def test_get_document_metadata_concurrent_requests(authenticated_client: AsyncClient):
...
@pytest.mark.anyio
@pytest.mark.api
async def test_get_citation_network_invalid_sample_size(authenticated_client: AsyncClient):
...
@pytest.mark.api
@pytest.mark.integration
async def test_get_citation_network_with_min_shared_refs(authenticated_client: AsyncClient):
...
@pytest.mark.api
@pytest.mark.integration
async def test_get_citation_network_concurrent_requests(authenticated_client: AsyncClient):
...
async def test_metadata_and_document_consistency(authenticated_client: AsyncClient):
    ...
    if list_response.status_code == 200:
        ...
        if docs:
            ...
            # Get metadata
            meta_response = await authenticated_client.get(f"/documents/{doc_id}/metadata")
