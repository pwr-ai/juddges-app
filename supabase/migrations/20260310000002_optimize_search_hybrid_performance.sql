-- =============================================================================
-- Migration: Optimize search_judgments_hybrid for large datasets
-- =============================================================================
-- Root causes observed in production-like data (~12k judgments):
-- 1) query shape caused planner to miss FTS partial indexes in common paths
-- 2) ranking recomputed to_tsvector(title+summary+full_text) for thousands
--    of hits, dominating execution time and causing statement timeouts
--
-- This migration keeps the API contract unchanged while:
-- - using index-friendly per-language text branches (UK/PL)
-- - keeping full-text matching on title+summary+full_text
-- - ranking with a cheaper title+summary vector
-- - preserving vector search, fuzzy fallback, and response schema
-- =============================================================================
...
COMMENT ON FUNCTION public.search_judgments_hybrid IS
    'Optimized hybrid search with language-specific index-friendly text branches, '
