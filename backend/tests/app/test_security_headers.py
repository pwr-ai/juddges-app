...
class TestSecurityHeaders:
    ...
    async def test_strict_transport_security_header(
        self,
        client: AsyncClient
    ):
    ...
    async def test_no_server_version_leakage(
        self,
        client: AsyncClient
    ):
    ...
    async def test_no_powered_by_header(
        self,
        client: AsyncClient
    ):
...
class TestCORSConfiguration:
    ...
    async def test_cors_disallows_unauthorized_origins(
        self,
        client: AsyncClient
    ):
        ...
        if response.status_code == 200:
            ...
            for method in expected_methods:
                assert method in methods, \
                    f"CORS should allow {method} method"

    async def test_cors_allowed_headers(
        self,
        client: AsyncClient
    ):
        ...
        if response.status_code == 200:
            ...
            for header in required_headers:
                assert header in allowed_headers, \
                    f"CORS should allow {header} header"
        ...
        if response.status_code == 200:
            ...
            if max_age:
                ...
                assert int(max_age) > 0, \
                    "Access-Control-Max-Age should be positive"
