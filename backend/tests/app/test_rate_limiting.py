...
class TestRateLimiting:
            ...
            if header in headers:
                ...
                if header != "x-ratelimit-reset":
                    assert value.isdigit(), \
                        f"{header} should be numeric: {value}"
    ...
    async def test_rate_limit_per_ip_or_per_key(
        ...
        rate_limited_same_key = any(r.status_code == 429 for r in responses_same_key)
...
class TestRateLimitBypass:
    ...
    async def test_authenticated_users_higher_limits(
        ...
        auth_limited = any(r.status_code == 429 for r in auth_responses)
        unauth_limited = any(r.status_code == 429 for r in unauth_responses)
...
class TestRateLimitSecurity:
    ...
    async def test_rate_limit_prevents_brute_force(
        self,
        client: AsyncClient
    ):
