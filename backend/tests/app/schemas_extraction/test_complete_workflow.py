...
class TestCompleteSchemaWorkflow:
    ...
    async def test_schema_to_extraction_workflow(
        ...
        job_id = extraction_response.json().get("id") or extraction_response.json().get("job_id")
        
        # 4. Check extraction status
        status_response = await client.get(
            f"/api/extractions/{job_id}",
            headers=auth_headers
        )
...
@pytest.mark.integration
@pytest.mark.skipif(not pytest.config.getoption("--run-ai-tests", default=False), reason="AI tests require --run-ai-tests flag")
class TestAIGenerationWorkflow:
    ...
    async def test_ai_generation_to_extraction(
        ...
        generated_schema = gen_response.json().get("schema")
    ...
    async def test_simple_ai_generation_workflow(
        ...
        gen_response = await client.post(
            ...
            json={
                "description": "Extract contract parties and dates"
            },
            headers=auth_headers
        )
...
class TestBulkExtractionWorkflow:
    ...
    async def test_bulk_extraction_complete_workflow(
        ...
        bulk_job_id = bulk_response.json().get("job_id") or bulk_response.json().get("id")
        
        # 3. Monitor status
        status_response = await client.get(
            f"/api/extractions/{bulk_job_id}",
            headers=auth_headers
        )
        assert status_response.status_code == 200
...
class TestSchemaVersioningWorkflow:
    ...
    async def test_rollback_workflow(
        ...
        rollback_response = await client.post(
            f"/api/schemas/db/{schema_id}/versions/1/rollback",
            headers=auth_headers
        )
...
class TestErrorHandlingWorkflow:
    ...
    async def test_extraction_retry_workflow(
        ...
        extraction_response = await client.post(
            ...
            json={
                "schema_id": schema_id,
                "document_id": "potentially-failing-doc"
            },
            headers=auth_headers
        )
        job_id = extraction_response.json().get("id") or extraction_response.json().get("job_id")
