...
export function SearchableDropdownButton({
 ...
 size="sm"
          className={cn(
            'group relative h-11 text-xs sm:text-sm font-medium gap-1.5 px-3 sm:px-4 rounded-none w-full justify-start',
            'bg-parchment',
            'shadow-none text-ink-soft',
            'hover:text-ink',
            ...
            value && [
              'border-l-2 border-l-oxblood border-rule',
              'bg-parchment-deep',
        ...
        >
          ...
          <span className="relative flex items-center gap-1.5 flex-1 min-w-0">
            ...
            <span className={cn(
              ...
              value && "text-ink font-semibold"
            )}>{displayLabel}</span>
            <ChevronDown
              size={12}
              className={cn(
                'opacity-80 group-hover:opacity-100 transition-transform duration-150 shrink-0',
                isOpen && '-rotate-180',
                value && 'text-oxblood'
              )}
            />
 ...
 className={cn(
          "w-[calc(100vw-2rem)] sm:w-[var(--radix-dropdown-menu-trigger-width)] sm:min-w-[20rem] rounded-none",
          "bg-parchment",
          "border border-rule",
          "shadow-md",
          "p-3",
      ...
      >
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className={cn(
              "w-full h-9 pl-9 pr-8 rounded-none",
              "bg-parchment",
              "border border-rule",
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-2 top-1/2 transform -translate-y-1/2 h-5 w-5 flex items-center justify-center rounded-none hover:bg-parchment-deep transition-colors"
        ...
        >
          {filteredOptions.length === 0 ? (
            <div className="px-4 py-6 text-center text-xs font-mono text-ink-soft" role="status">
              No results found
            </div>
          ) : (
            filteredOptions.map((option, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full h-10 px-3 rounded-none flex items-center justify-between",
                ...
                >
                  <div className="flex items-center gap-2.5 flex-1 min-w-0 text-left">
                    {icon && (
                      <span className="shrink-0 text-ink-soft [&>svg]:h-4 [&>svg]:w-4">
                        {icon}
                      </span>
                    )}
                    <div className="flex flex-col gap-0.5 flex-1 min-w-0 text-left">
                      <span className="text-xs font-medium truncate text-left">{option.label}</span>
                      {option.description && (
                        <span className="text-[11px] text-ink-soft line-clamp-1 text-left font-mono">
                          {option.description}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {option.badge && (
                      <div className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-none text-[10px] font-mono bg-parchment-deep text-ink-soft border border-rule">
