...
const renderBadgeOrSpan = (content: string, key: string | number, isLegalReferences: boolean) => {
  if (isLegalReferences) {
    return (
      <div
        key={key}
        className="block rounded-md border px-2 py-0.5 text-xs font-medium break-words bg-slate-100/60 dark:bg-slate-800/60 border-slate-200/30 dark:border-slate-700/30 text-foreground"
